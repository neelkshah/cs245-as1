...
public class IndexedRowTable implements Table {
    ...
    public long predicatedColumnSum(int threshold1, int threshold2) {
        long sum = 0;
        if (indexColumn == 1) {
            for (IntArrayList iteration : index.tailMap(threshold1 + 1).values()) {
                for (int rowId : iteration) {
            ...
            for (IntArrayList iteration : index.headMap(threshold2).values()) {
                for (int rowId : iteration) {
    ...
    /**
     * Implements the query
     *  SELECT SUM(col0) + SUM(col1) + ... + SUM(col_n) FROM table WHERE col0 > threshold;
     *
    ...
    public long predicatedAllColumnsSum(int threshold) {
        long sum = 0;
        if (indexColumn == 0) {
            for (IntArrayList iteration : index.tailMap(threshold + 1).values()) {
                for (int rowId : iteration) {
                    for (int colId = 0; colId < numCols; colId++) {
    ...
    public int predicatedUpdate(int threshold) {
        int updatedRows = 0;
        if (indexColumn == 0) {
            for (IntArrayList iteration : index.headMap(threshold).values()) {
                updatedRows += iteration.size();
                for (int rowId : iteration) {
